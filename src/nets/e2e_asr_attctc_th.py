    ...
    def __init__(self, idim, odim, args):
        ...
        if args.atype == 'noatt':
            ...
            self.att = AttMultiHeadCnnAdd(args.eprojs, args.dunits,
                                          args.aheads, args.adim, args.adim)
        elif args.atype == 'multi_head_cnn_add_v1':
            self.att = AttMultiHeadCnnAddv1(args.eprojs, args.dunits,
                                          args.aheads, args.adim, args.adim)
        elif args.atype == 'multi_head_cnn_add_v2':
            self.att = AttMultiHeadCnnAddv2(args.eprojs, args.dunits,
                                            args.aheads, args.adim, args.adim)
...
class AttMultiHeadCnnAdd(torch.nn.Module):
    ...
    def __init__(self, eprojs, dunits, aheads, att_dim_k, att_dim_v):
        ...
        for h in six.moves.range(aheads):
            ...
            # for testing, this number should match with regular AttMultiHeadAdd
            self.cnn_k += [torch.nn.Conv2d(1, att_dim_k, kernel_size=(1, eprojs),
                                           stride=1, padding=(0, 0), dilation=(1, 1), bias=True)] # padding = (kernel_size//2 + dilation//2, 0) to be same number of frames as original
            self.mlp_v += [torch.nn.Linear(eprojs, att_dim_v, bias=False)]
            self.gvec += [torch.nn.Linear(att_dim_k, 1)]
        self.mlp_o = torch.nn.Linear(aheads * att_dim_v, eprojs, bias=False)
        self.dunits = dunits
...
class AttMultiHeadCnnAddv1(torch.nn.Module):
    # same as AttMultiHeadCnnAdd, with diff config
    #   head0 is same, head1 has a 3 kernel cnn
    #       hopefully the performance will be same may be slightly better than AttMultiHeadCnnAdd
    ...
    def __init__(self, eprojs, dunits, aheads, att_dim_k, att_dim_v):
        super(AttMultiHeadCnnAddv1, self).__init__()
        self.mlp_q = torch.nn.ModuleList()
        # self.mlp_k = torch.nn.ModuleList()
        self.cnn_k = torch.nn.ModuleList()
        self.mlp_v = torch.nn.ModuleList()
        ...
        for h in six.moves.range(aheads):
            ...
            #                                stride=1, padding=(3//2) * (h+1), dilation=h+1, bias=True)] # padding = (kernel_size//2 + dilation//2, 0) to be same number of frames as original
            # for testing, this number should match with regular AttMultiHeadAdd
            self.cnn_k += [torch.nn.Conv2d(1, att_dim_k, kernel_size=(2*h+1, eprojs),
                                           stride=1, padding=(h, 0), dilation=(1, 1), bias=True)] # padding = (kernel_size//2 + dilation//2, 0) to be same number of frames as original
            self.mlp_v += [torch.nn.Linear(eprojs, att_dim_v, bias=False)]
            self.gvec += [torch.nn.Linear(att_dim_k, 1)]
        self.mlp_o = torch.nn.Linear(aheads * att_dim_v, eprojs, bias=False)
        self.dunits = dunits
...
class AttMultiHeadCnnAddv2(torch.nn.Module):
    # same as AttMultiHeadCnnAdd, with diff config
    #   head0 is same, head1 has a 3 kernel cnn
    #       hopefully the performance will be same may be slightly better than AttMultiHeadCnnAdd
    ...
    def __init__(self, eprojs, dunits, aheads, att_dim_k, att_dim_v):
        super(AttMultiHeadCnnAddv2, self).__init__()
        self.mlp_q = torch.nn.ModuleList()
        # self.mlp_k = torch.nn.ModuleList()
        self.cnn_k = torch.nn.ModuleList()
        self.mlp_v = torch.nn.ModuleList()
        ...
        for h in six.moves.range(aheads):
            ...
            (kernel_size, dilation)=(2*h+1, h+1)
            padding = kernel_size//2 * dilation
            self.cnn_k += [torch.nn.Conv2d(1, att_dim_k, kernel_size=(kernel_size, eprojs),
                                           stride=1, padding=(padding, 0), dilation=(dilation, 1), bias=True)] # padding = (kernel_size//2 + dilation//2, 0) to be same number of frames as original
            self.mlp_v += [torch.nn.Linear(eprojs, att_dim_v, bias=False)]
            self.gvec += [torch.nn.Linear(att_dim_k, 1)]
        self.mlp_o = torch.nn.Linear(aheads * att_dim_v, eprojs, bias=False)
        self.dunits = dunits
    ...
    def calculate_all_attentions(self, hpad, hlen, ys):
        ...
        if isinstance(self.att, AttLoc2D):
            ...
            att_ws = torch.stack([aw[0] for aw in att_ws], dim=1).data.cpu().numpy()
                                   ...
                                   AttMultiHeadCnnAdd, AttMultiHeadCnnAddv1, AttMultiHeadCnnAddv2)):
