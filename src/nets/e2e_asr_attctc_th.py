...
def _get_maxpooling2_odim(idim, in_channel=3, out_channel=128, ceil_mode=False, mode='regular'):
...
class E2E(torch.nn.Module):
    def __init__(self, idim, odim, args):
        ...
        if args.etype == 'blstmp' or args.etype == 'bgrup' \
                ...
                or args.etype == 'blstmp_vggblstm_v3':
        ...
        if args.atype == 'noatt':
            ...
            self.att = AttMultiHeadAdd(args.eprojs, args.dunits,
                                       args.aheads, args.adim, args.adim)
        elif args.atype == 'multi_head_cnn_add':
            self.att = AttMultiHeadCnnAdd(args.eprojs, args.dunits,
                                          args.aheads, args.adim, args.adim)
...
class AttMultiHeadCnnAdd(torch.nn.Module):
    '''Multi head additive attention
    ...
    def __init__(self, eprojs, dunits, aheads, att_dim_k, att_dim_v):
        super(AttMultiHeadCnnAdd, self).__init__()
        self.mlp_q = torch.nn.ModuleList()
        # self.mlp_k = torch.nn.ModuleList()
        self.cnn_k = torch.nn.ModuleList()
        self.mlp_v = torch.nn.ModuleList()
        self.gvec = torch.nn.ModuleList()
        for h in six.moves.range(aheads):
            self.mlp_q += [torch.nn.Linear(dunits, att_dim_k)]
        ...
        # self.cum_elapsed_time = 0.0
        # self.cum_elapsed_time_cnt = 0
    ...
    def apply_cnn_head_transform(self, cnn, x):

        xs = x.view(x.size(0), 1, x.size(1), x.size(2))
        return cnn(xs).squeeze(-1).transpose(1, 2)
    ...
    def forward(self, enc_hs_pad, enc_hs_len, dec_z, att_prev):
        ...
        if self.pre_compute_k is None:
            ...
            # utt x frame x att_dim
            # st_time = time.time()
            # enc_h_reshaped = self.enc_h.view(self.enc_h.size(0), 1, self.enc_h.size(1), self.enc_h.size(2))
            # self.pre_compute_k = [
            #     self.cnn_k[h](enc_h_reshaped).squeeze(-1).transpose(1, 2) for h in six.moves.range(self.aheads)]

            enc_h_reshaped = self.enc_h.view(self.enc_h.size(0), self.enc_h.size(2), self.enc_h.size(1))
            self.pre_compute_k = [
...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, etype, idim, elayers, eunits, eprojs, subsample, dropout, in_channel=1):
        ...
        if etype == 'blstm':
        ...
        elif etype == 'vggblstm_ruizhili_v1_20180723':
            ...
            self.enc2 = BLSTM(_get_maxpooling2_odim(idim, in_channel=in_channel),
                              elayers, eunits, eprojs, dropout)
            logging.info('Use CNN-VGG + BLSTM ruizhili_v1_20180723 for encoder')
        elif etype == 'bgrup':
            ...
            logging.info('Use MultiEncoders BLSTMP and VGGBLSTM, for encoder')
        elif etype == 'blstmp_vggblstm_v3':
            self.enc_blstmp = BLSTMP(idim, elayers, eunits,
                                     eprojs, subsample, dropout)
            self.enc_blstmp_proj = torch.nn.Linear(eunits, eunits)

            self.enc_vgg = VGG2L(in_channel)
            self.enc_vgg_blstmp = BLSTM(_get_vgg2l_odim(idim, in_channel=in_channel),
                                        elayers, eunits, eprojs, dropout)
            self.enc_vgg_blstmp_proj = torch.nn.Linear(eunits, eunits)
    ...
    def forward(self, xs, ilens):
        ...
        if self.etype == 'blstm':
            ...
            xs, ilens = self.enc1(xs, ilens)
            xs, ilens = self.enc2(xs, ilens)
        elif self.etype == 'vggblstm_ruizhili_v1_20180723':
            xs, ilens = self.enc1(xs, ilens)
            xs, ilens = self.enc2(xs, ilens)
        ...
        elif self.etype == 'blstmp_vggblstm_v3':

            # encoder1, blstmp
            xs_blstmp, ilens_blstmp = self.enc_blstmp(xs, ilens)
            xs_blstmp_proj = self.enc_blstmp_proj(xs_blstmp)

            # encoder2, vgg->blstmp
            xs_vgg, ilens_vgg = self.enc_vgg(xs, ilens)
            xs_vgg_blstmp, ilens_vgg_blstmp = self.enc_vgg_blstmp(xs_vgg, ilens_vgg)
            xs_vgg_blstmp_proj = self.enc_vgg_blstmp_proj(xs_vgg_blstmp)

            xs = torch.add(xs_blstmp_proj, xs_vgg_blstmp_proj) # simpled adding of two encoders
...
class VGG(torch.nn.Module):
    ...
    def __init__(self, batch_norm=False, spatial_dp=False, ceil_mode=False, in_channels=1,inplace=True, bias=False):
        super(VGG, self).__init__()

        # self.conv0 = ConvBasicBlock(in_channels, 16, 1, 1, 0, False, True, batch_norm, spatial_dp)
        self.resblock1 = CNNBasicBlock(in_channels, 64, batch_norm=batch_norm, spatial_dp=spatial_dp, resnet=False,inplace=inplace, bias=bias)
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=ceil_mode)

        self.resblock2 = CNNBasicBlock(64, 128, batch_norm=batch_norm, spatial_dp=spatial_dp, resnet=False,inplace=inplace, bias=bias)
...
class CNNBasicBlock(torch.nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, batch_norm=False, spatial_dp=False, resnet=False, inplace=True, bias=False):
        ...
        self.conv1 = ConvBasicBlock(in_channels, out_channels, 3, stride, 1, bias, True, batch_norm, spatial_dp, inplace=inplace)
        self.conv2 = ConvBasicBlock(out_channels, out_channels, 3, 1, 1, bias, False, batch_norm, spatial_dp, inplace=inplace)
        self.relu = torch.nn.ReLU(inplace=True)
        if (in_channels != out_channels or stride != 1) and resnet:
            self.downsample = ConvBasicBlock(in_channels, out_channels, 1, stride, 0, bias, False, batch_norm, spatial_dp, inplace=inplace)
...
class ConvBasicBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=True, nonlinear=False, batch_norm=False, spatial_dp=False, maxpooling=False, maxpool_kernel_size=3, maxpool_stride=2, maxpool_padding=1, maxpool_ceil_mode=False, inplace=True):
